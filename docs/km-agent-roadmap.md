...
- AI co-editing now renders a safe whiteboard revision while the model is still responding instead of waiting for one complete JSON payload. A bounded JSON-lines protocol validates each finished node, connection, and note event, progressively redraws an isolated `AI draft` board, promotes it only after a final `done` event and full graph validation, and falls back to the original proposal format for older models and conversations. Every request is pinned to its send-time source board: switching elsewhere keeps that canvas active while the draft continues in background storage, reopening the dialog can recover a pending request, and deleting the source fails closed instead of modifying an unrelated board. Board tabs and the copilot header persist generating/ready attention, opening a version clears its unread state, repeated AI titles receive stable numeric suffixes, and explicit designer navigation wins the first-frame race without mutating the newly selected board.
...
- AI whiteboard proposals now include a Codex-style pre-apply review against the current scene. Designers see added and removed nodes, links, and notes plus game-flow checks for starts, ends, branches, loops, disconnected or unreachable content, incomplete decisions, accidental dead ends, and failures without retry exits. A game-design-specific playtest readiness score also checks six minimum-demo elements—objective, core interaction, outcome decision, feedback, failure recovery, and reachable completion—without blocking rapid Build attempts. When gaps remain, one action opens the safe version and asks AI to add only the missing playable-loop elements; the revised score updates live. The comparison baseline is captured before automatic or manual application, so the review remains meaningful after a safe revision is created.
...
- Flow playtests now close the iteration loop inside the canvas. A designer can classify the actual run as unclear guidance, weak feedback, a stuck state or soft lock, a pacing issue, or a weak branch; add bounded reproduction detail; and send the exact clicked trace as hidden context to the whiteboard copilot. KM Agent immediately opens the live copilot and requests a safe revision while preserving unaffected flow. Legacy task history containing early percent-encoded Markdown data attachments is recovered as text before model replay, so one malformed historical whiteboard turn no longer permanently breaks every later AI edit.
